...
for n1 = 1:n_neurons
    for n2 = (n1+1):n_neurons
        ...
        % for all combinations of trials
        for trial1 = 1:n_trials
            for trial2 = 1:n_trials

                % pick spike trains for this iteration
                spikes_temp1 = squeeze(spikes(trial1, :, n1));
                spikes_temp2 = squeeze(spikes(trial2, :, n2));
                spikes_temp = [spikes_temp1; spikes_temp2]';

                % find synchrony
                temp = cps_sw( spikes_temp, triangleWidth, binSize, binShift, stepSize );

                % store synchrony for later
                if trial1 == trial2
                    % If the trial numbers are equal, then this is the
                    % actual pairing. We should NOT include this in our
                    % bootstrapping, but we SHOULD do the calculation and
                    % store it someplace else as the actual (NOT
                    % bootstrapped) value
