...
% be run on a computer you are sitting at to see the results. This is the
% same as version 3b, except it does not pause between plots and saves all
% figures as jpegs.

clear all
close all

filename = 'results_20090226_160344.mat'
...
for n1 = 1:n_neurons
    for n2 = (n1+1):n_neurons
        ...
        plot(t,binned(:,n1) - 2,'-g')
        plot(t,binned(:,n2) - 3,'-g')
...
%        legend('95% Confidence', '     Interval','Synchrony','PSTH', 'PSTH')
        ...
        filename = [textdata{col_trial} '__' int2str(n1) 'vs' int2str(n2) '.jpg'];
        saveas(fh,filename) 
