% Show that conv-corr method is dependent on firing rate by trying all
...
% synchrony levels and not at different firing rates. This version uses
% cps2, which takes the correlation instead of the covariance.
    ...
    for jj = 1:length(rates)
        for kk = jj:length(rates)
            ...
            for mm = 1:nChunks
                ...
                [s leftovers] = cps2(spikes(rows,:), triangleWidth, leftovers);
