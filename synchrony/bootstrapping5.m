...
% 
% This particular version uses makeSpikeTrainsGamma which makes
% gamma-distrubuted (as opposed to poisson distributed) spike trains Other
% than that, it should be exactly the same as bootstrapping4
...
for ii = 1:length(sync)
    ...
    % Make spike trains
    spikes = makeSpikeTrainsGamma(rates, sync(ii), trainLength, stepSize, offset);
    ...
    if any(isnan(meanSyncNormal)) || any(isnan(meanSyncShuffled))
        warning('some mean synchrony is NaN') 
    end
