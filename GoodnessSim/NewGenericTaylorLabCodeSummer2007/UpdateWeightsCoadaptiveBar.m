...
% if 2 blocks had the same pct hit, then the one with the Closest mean endpoint distance is
% used.
...
for i=1:decode.num_SpRerefChan;
    for n=1:VR.num_dim
        fprintf(fpCO,'%f\t',  Parameters.Wp(i,n,iUpdateBlock));
    end
    for n=1:VR.num_dim
        fprintf(fpCO,'%f\t',  Parameters.Wn(i,n,iUpdateBlock));
    end
    fprintf(fpCO,'%f\n',   Parameters.estNpower(i));
end
% print stuff out
