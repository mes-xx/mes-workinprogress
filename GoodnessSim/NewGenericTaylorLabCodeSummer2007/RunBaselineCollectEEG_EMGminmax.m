...
%Parameters.meanpower same length as data.power_buffer???
 EMGcolumns=length(data.power_buffer)-length(SigProc.freq_bin);
for i=1:VR.num_targets
    for j=1:length(data.power_buffer(:,1,i))
    MeanNormalized(j,i)=mean((data.power_buffer(j,1:EMGcolumns,i)-Parameters.meanpower(1:EMGcolumns))./Parameters.std2power(1:EMGcolumns));
    end
