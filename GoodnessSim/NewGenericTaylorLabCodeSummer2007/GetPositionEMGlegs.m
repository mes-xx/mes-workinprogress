...
VR.cursor_position(1)=VR.movement.range*(mean(data.normalized((length(data.normalized)-length(SigProc.freq_bin)+1):length(data.normalized)))/Parameters.range);%plus scaling
