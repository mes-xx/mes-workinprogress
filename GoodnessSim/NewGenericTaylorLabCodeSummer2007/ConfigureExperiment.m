    ...
    while(line~=-1)
        while KeepRunningFlag 
            ...
            elseif ~isempty(findstr(line,'Data_Location'))==1
                ...
                FOptoIn = [line(length('Data_Location ')+1:length(line)) Subject_Code '\'];
                a = exist(FOutBase);
                if a~=7
                    mkdir(FOutBase)
                end
                ...
                FOptoFileName = [FOptoIn 'OptoCenter.dat'];
...
%                         fprintf(fpCO,'\n');
           ...
           elseif ~isempty(findstr(line,'VR.Freq'))==1
                VR.Freq =str2num(line(length('VR.Freq')+1:length(line)));
                fprintf(fpCO,'VR.Freq\t%2.5f\n',VR.Freq);
           elseif ~isempty(findstr(line,'VR.WavesPerTarget'))==1
                VR.WavesPerTarget =str2num(line(length('VR.WavesPerTarget')+1:length(line)));
                fprintf(fpCO,'VR.WavesPerTarget\t%2.5f\n',VR.WavesPerTarget);
           ...
           elseif ~isempty(findstr(line,'Parameters.BaselineFilePath'))==1
                Parameters.BaselineFilePath =str2num(line(length('Parameters.BaselineFilePath')+1:length(line)));
                fprintf(fpCO,'Parameters.BaselineFilePath\t%i\n',Parameters.BaselineFilePath);
           ...
           elseif ~isempty(findstr(line,'SigProc.ElectrodeList'))==1% for a given experimental EEG setup, you should have an electrode name list eg (CZ C1 C3 F3, Cp2...)so that we can easily relate weights and values to actual electrode locations
            ...
            elseif ~isempty(findstr(line,'SigProc.ReRefMat'))==1% For a given EEG experimental set up, this is the .txt file that is read in and defines the referencing (e.g. CAR, Laplacian, etc)
                ...
                if strcmp(rerefname,'eye')
                    SigProc.ReRefMat=eye(TDT.num_chan);
                else
            ...
            elseif ~isempty(findstr(line,'SigProc.SpReRefMat'))==1% For a given EEG experimental set up, this is the .txt file that is read in and defines the referencing (e.g. CAR, Laplacian, etc)
                ...
                fprintf(fpCO,'SigProc.ReRefMat \t%s\n',rerefname);
                if strcmp(rerefname,'eye')
                    SigProc.SpReRefMat=zeros(4, TDT.num_chan);
					SigProc.SpReRefMat(1, :)=ones(1, TDT.num_chan);
                else
