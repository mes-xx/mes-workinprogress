...
%    TotalBaselineTimeInSamples=(timing.mean_windowbyTarg*VR.num_targets*TDT.sample_rate*timing.loop_intervalS)+timing.now;
    ...
    while timing.now<timing.mean_windowS
...
% 			end
            ...
            timing.Targ_endS=timing.now+timing.max_movement_timeS;
		
      
			% read data from teh TDT and write it to file for the amt of time
			% calculated above
            while timing.now<timing.Targ_endS
				% collect data
                eval(Funk.GetData);
				% write data to file
...
fprintf(fpCO,'WeightList\n');
 ...
 % print stuff out
for i=1:decode.num_SpRerefChan;
    fprintf(fpCO,'%i\t%f\t', i, Parameters.meanpower(i));
    fprintf(fpCO,'%f\t%f\n',   Parameters.std2power(i),Parameters.estNpower(i));
end
