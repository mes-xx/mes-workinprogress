...
% information content for each neuron
info = (Neurons.PrefDirection' * targs')';
noise = Neurons.NoiseScale .* (rand(1, TDT.num_chan) * Neurons.Correlation);
% noiseX=A*randn(1);
% Xinfo=C*targs(:,1);
...
data.power_buffer(iTargetTally(VR.target_num),:, VR.target_num)=info + noise;
